...
uses
  SDL2, SysUtils, Classes, data_loaders, uMapping, uGameTypes, renderer, uMonsterAI, uGameLogic;
...
function GetMonsterTileID(const S: TWorldState; const M: TSetMonster): Integer;
...
begin
  ...
  // Load initial monster data and pictures for the world map
  LoadMonsterPictures(DataPath(DataDir, 'landmon.con'), R.FSDLRenderer, S.monsterTiles);
  LoadMonstersForMap(S);
...
var
  baseID: Integer;
begin
  if (M.OriPic < 0) or (M.OriPic >= S.monsterTiles.Count) then
  begin
    Result := -1;
    exit;
  end;
  ...
  // Handle animation: even IDs are frame 1, odd are frame 2
  if S.animAltNow and (baseID + 1 < S.monsterTiles.Count) then
  begin
    // Check if the next tile is the animated pair for the current one
    // This is a simple convention: frame 2 is at index+1
    Result := baseID + 1;
  end
  else
  begin
...
begin
  ...
  // Update monster statuses
  if not S.MonstersPaused then
  begin
    for i := 0 to High(S.SetMonsters) do
    begin
      if S.SetMonsters[i].Status = msNormal then
      begin
        dist := Sqrt(Sqr(S.Player.Xloc - S.SetMonsters[i].X) + Sqr(S.Player.Yloc - S.SetMonsters[i].Y));
        if dist <= CHASE_RADIUS then
        begin
          S.SetMonsters[i].Status := msChasing;
        end;
      end;
    end;
  end;
...
begin
  ...
  for i := 0 to High(S.SetMonsters) do
  begin
    // only draw monsters that are visible
    if (not S.VisibilityEnabled) or S.visibilityMap.Data[S.SetMonsters[i].Y * S.visibilityMap.Width + S.SetMonsters[i].X] then
    begin
      screenX := S.SetMonsters[i].X * tw - S.cameraX;
      screenY := S.SetMonsters[i].Y * th - S.cameraY;

      monsterTileID := GetMonsterTileID(S, S.SetMonsters[i]);

      if monsterTileID <> -1 then
        DrawTile(R.FSDLRenderer, S.monsterTiles, monsterTileID, screenX, screenY);
    end;
  end;
...
begin
  ...
  case NewMapType of
    mtWorld:
    begin
      S.worldIDs := LoadMAPOrdered(DataPath(S.DataDir, 'WORLD.MAP'), 110, 100, 1, True);
      LoadCON(DataPath(S.DataDir, 'UNIV.CON'), S.univPixels, S.univCount, False, True);
      LoadCON(DataPath(S.DataDir, 'LAND.CON'), S.landPixels, S.landCount, False, True);
      ...
      BuildDrawMapForWorld(S.drawMapB, S, True);
      BuildReverseMap(S);
      LoadMonsterPictures(DataPath(S.DataDir, 'landmon.con'), R.FSDLRenderer, S.monsterTiles);
    end;
    mtTown, mtCastle:
    begin
      ...
      LoadMonsterPictures(DataPath(S.DataDir, 'townmon.con'), R.FSDLRenderer, S.monsterTiles);
    ...
    begin
      ...
      BuildReverseMap(S);
      LoadMonsterPictures(DataPath(S.DataDir, 'dungmon.con'), R.FSDLRenderer, S.monsterTiles);
    end;
  end;
...
begin
  ...
  begin
    case E.key.keysym.scancode of
      ...
      SDL_SCANCODE_P: S.MonstersPaused := not S.MonstersPaused;
      SDL_SCANCODE_V:
      begin
        S.VisibilityEnabled := not S.VisibilityEnabled;
        writeln('VisibilityEnabled toggled to: ', S.VisibilityEnabled);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Monsters only move if the player moves into a visible tile, or if visibility is off
      if (not S.VisibilityEnabled) or S.visibilityMap.Data[NewY * S.visibilityMap.Width + NewX] then
      begin
        if not S.MonstersPaused then MonsterMovement(S);
      end;
    end;
  end
  else // On world map
